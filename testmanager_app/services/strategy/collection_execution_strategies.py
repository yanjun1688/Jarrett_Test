...
class CollectionExecutionStrategyInterface(ABC):
    ...
    def execute_in_worker(
        ...
        默认实现使用 async_to_sync 调用异步 execute 方法。
        链式执行策略会覆盖此方法使用纯同步实现。
        """
        return async_to_sync(self.execute)(collection_requests, user, collection_exec, context)
...
class ChainExecutionStrategy(CollectionExecutionStrategyInterface):
        ...
        for coll_req in normal_requests:
            ...
            for i in range(request_count):
                try:
                    ...
                    result = execute_request_direct(rendered_request)
                    executions.append(result)
    ...
    def _execute_full_sync(self, collection_requests: List, user: Any, collection_exec: Any, context: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        # 加载场景变量到 context（P0 功能）
        if collection_exec and hasattr(collection_exec, 'collection'):
            collection = collection_exec.collection
            if hasattr(collection, 'variables') and collection.variables:
                for var in collection.variables:
                    if isinstance(var, dict) and 'key' in var and 'value' in var:
                        context[var['key']] = var['value']
                        logger.info(f"[Chain Execution] Loaded scenario variable: {var['key']} = {var['value']}")
...
class CollectionExecutionStrategyFactory:
    ...
    def get_strategy(cls, execution_mode: str) -> CollectionExecutionStrategyInterface:
        ...
        # 确保_strategies已初始化
        if cls._strategies is None:
            raise RuntimeError("策略工厂初始化失败：_strategies为None")
