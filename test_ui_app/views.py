...
class UITestScriptViewSet(viewsets.ModelViewSet):
    ...
    def preview_page(self, request: Request) -> Response:
        ...
        - viewport_width: 视口宽度 (可选, 默认1280)
        - viewport_height: 视口高度 (可选, 默认720)
        """
        ...
        browser_type = request.query_params.get('browser_type', 'chromium')
        viewport_width = int(request.query_params.get('viewport_width', 1280))
        viewport_height = int(request.query_params.get('viewport_height', 720))
        
        try:
            # 使用PlaywrightService预览页面
            ...
            if result.get('success'):
                ...
                return Response(
                    {'error': result.get('error', '预览页面失败')},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
        except Exception as e:
            logger.error(f"预览页面失败: {str(e)}")
            return Response(
                {'error': f'预览页面失败: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def select_element(self, request: Request) -> Response:
        ...
        - x: 点击的X坐标 (可选)
        - y: 点击的Y坐标 (可选)
        - selector: 选择器 (可选, 如果提供坐标则不需要)
        - browser_type: 浏览器类型 (可选)
        - viewport_width: 视口宽度 (可选)
        - viewport_height: 视口高度 (可选)
        ...
        if not x and not y and not selector:
            return Response(
                {'error': '需要提供坐标(x,y)或选择器(selector)'},
                status=status.HTTP_400_BAD_REQUEST
            )
        
        try:
            # 使用PlaywrightService选择元素
            from .services import PlaywrightService
            service = PlaywrightService()
            result = service.select_element_sync(
                url=url,
                x=x,
                y=y,
                selector=selector,
            ...
            if result.get('success'):
                ...
                return Response(
                    {'error': result.get('error', '选择元素失败')},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
        except Exception as e:
            logger.error(f"选择元素失败: {str(e)}")
            return Response(
                {'error': f'选择元素失败: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
