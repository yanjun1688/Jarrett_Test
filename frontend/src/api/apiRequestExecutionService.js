...
export const apiRequestExecutionAPI = {
  ...
  // 批量执行状态获取
  batchExecuteStatus: (executionIds) => {
    return apiClient.post('/executions/batch-status/', { execution_ids: executionIds });
  },
  
  // 通过执行ID获取执行记录
  getExecutionById: (executionId) => {
    return apiClient.get(`/test-executions/${executionId}/`);
  },
  
  // 获取最新执行历史
  getLatestExecutionHistory: (apiRequestId, limit = 10) => {
    return apiClient.get(`/test-executions/?api_request=${apiRequestId}&limit=${limit}&ordering=-executed_at`);
  }
};
