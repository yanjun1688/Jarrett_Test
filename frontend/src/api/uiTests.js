...
export const uiTestsAPI = {
  ...
  getScripts: (params = {}) => {
    ...
    return apiClient.get(`/ui-scripts/${queryString ? `?${queryString}` : ''}`);
  },
  ...
  // 执行脚本
  executeScript: (id) =>
    apiClient.post(`/ui-scripts/${id}/execute/`),

  // 同步录制（阻塞式）
  syncRecord: (data) =>
    apiClient.post('/ui-scripts/sync_record/', data, {
  ...
  // 获取执行日志详情
  getExecutionLogs: (id) =>
    apiClient.get(`/ui-executions/${id}/logs/`),
